...
| P0 | Analyze auditability representativeness | In progress | v1 stopped before output because an auxiliary fixed pre-window was incomplete for one eligible anchor. `configs/auditability_representativeness_v2.json` uses the eligibility-guaranteed complete prior Method Code run instead, with no outcome-based exclusion |
