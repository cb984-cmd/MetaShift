...
| P0 | Analyze auditability representativeness | In progress | v1 stopped before output because an auxiliary fixed pre-window was incomplete for one eligible anchor. v2 uses the eligibility-guaranteed complete prior Method Code run and retains the one state-code-80 anchor as Outside EPA mapped regions, with no outcome-based exclusion |
